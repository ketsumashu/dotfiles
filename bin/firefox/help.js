...
(() => {
  ...
  // src/help.ts
  function initTridactylSettingElem(elem, kind) {
    let bindingNode = elem.getElementsByClassName(`Tridactyl${kind}`)[0];
    if (bindingNode) {
      Array.from(bindingNode.children).filter((e) => e.tagName === "SPAN").forEach((e) => e.parentNode.removeChild(e));
    } else {
      bindingNode = document.createElement("p");
      bindingNode.className = `TridactylSetting Tridactyl${kind}`;
      bindingNode.textContent = kind + ":\xA0";
  ...
  async function addSetting(settingName) {
    const commandElems = getCommandElements();
    delete commandElems.composite;
    const settingElems = Object.keys(
      commandElems
    ).reduce((settingElems2, cmdName) => {
      settingElems2[cmdName] = initTridactylSettingElem(
        commandElems[cmdName],
        settingName
      );
      return settingElems2;
    }, {});
    const settings = await getAsyncDynamic(settingName);
    for (const setting of Object.keys(settings)) {
      let excmd = settings[setting].split(" ");
      excmd = ["composite", "fillcmdline", "current_url"].includes(excmd[0]) ? excmd[1] : excmd[0];
      while (settings[excmd]) {
        excmd = settings[excmd].split(" ");
        excmd = ["fillcmdline", "current_url"].includes(excmd[0]) ? excmd[1] : excmd[0];
      }
    ...
    Object.values(settingElems).filter(
      (e) => !Array.from(e.children).find((c) => c.tagName === "SPAN")
    ...
    browser.storage.onChanged.addListener((changes) => {
      if ("userconfig" in changes) {
        ...
        [...modeMaps, "exaliases"].forEach((kind) => {
          if (JSON.stringify(changes.userconfig.newValue[kind]) !== JSON.stringify(changes.userconfig.oldValue[kind]))
            addSetting(kind);
        });
      }
    });
  ...
  function addSettingInputs() {
    ...
    const inputClassNameModified = inputClassName + " TridactylSettingInputModified ";
    const onKeyUp = async (ev) => {
      const input = ev.target;
      if (ev.key === "Enter") {
        ;
        window.tri.messaging.message(
          "controller_background",
          "acceptExCmd",
          ["set " + input.name + " " + input.value]
        );
      } else {
        if (input.value === await getAsync(input.name.split("."))) {
    ...
    return Promise.all(
      Array.from(
        document.querySelectorAll("a.tsd-anchor")
      ).map(
        async (a) => {
          const section = a.parentNode;
          const settingName = a.name.split(".");
          const value = await getAsyncDynamic(...settingName);
          if (!value)
          ...
          addChangeListener(input.name, (_, newValue) => {
  ...
  function addResetConfigButton() {
    ...
    button.addEventListener("click", () => {
      ...
      if (p === sentence) {
        ;
        window.tri.messaging.message("controller_background", "acceptExCmd", sentence).then((_) => alert("Config reset!"));
      } else {
