...
(() => {
  ...
  async function updatePage() {
    ...
    article.content = (0, import_xss.default)(article.content, { stripIgnoreTag: true });
    const content = document.createElement("main");
    content.innerHTML = article.content;
    document.body.appendChild(content);
    if (article.title !== void 0) {
      const header = document.createElement("header");
      const title = document.createElement("h1");
      if (await getAsync("readerurlintitle") == "true" && !((_a = article.title) != null ? _a : "").includes(article.link)) {
        document.title = [article.siteName, article.title].filter(Boolean).join(": ") + " :: " + article.link;
      } else {
        document.title = [article.siteName, article.title].filter(Boolean).join(": ");
      }
      title.textContent = article.title;
      header.appendChild(title);
      if (article.byline !== void 0) {
    ...
    if (article.link !== void 0) {
      const link = (_b = document.getElementById("tricanonlink")) != null ? _b : document.createElement("link");
      link.rel = "canonical";
      link.id = "tricanonlink";
      link.href = article.link;
      document.head.appendChild(link);
    } else {
      (_c = document.getElementById("tricanonlink")) == null ? void 0 : _c.remove();
    }
  }
